...
- Markdown：mistune AST 直接生成 Delta（不经 HTML）
- HTML：lxml 解析（爬虫 --delta-from-html 时使用）
...
from md_ast_quill import markdown_ast_to_quill_delta
...
def download_image(
    ...
    dest: Path,
    user_agent: str,
    referer: str = "https://www.xiaolincoding.com/",
) -> None:
...
def _normalize_obs_key_prefix(prefix: str) -> str:
    """OBS object key 前缀，去掉首尾 /。"""
    return (prefix or "").strip().strip("/")


def _build_obs_object_key(prefix: str, ext: str) -> str:
    """生成 OBS object key：{prefix}/{uuid}{ext}。"""
    base = f"{uuid.uuid4().hex}{ext}"
    norm = _normalize_obs_key_prefix(prefix)
    return f"{norm}/{base}" if norm else base
...
async def migrate_markdown_images(
    ...
    *,
    ...
    cache_dir: Path,
    user_agent: str,
    obs_key_prefix: str = "wiki/import/xiaolincoding",
    referer: str = "https://www.xiaolincoding.com/",
    ...
    for url in extract_markdown_image_urls(markdown):
        ...
        try:
            await asyncio.to_thread(
                download_image, url, tmp_path, user_agent, referer
            )
            ext = _guess_ext(url, tmp_path)
            key = _build_obs_object_key(obs_key_prefix, ext)
        ...
        if li_ops:
            for op in li_ops:
                ...
                ops.append(
                    {
                        "attributes": attrs,
                        "insert": insert if isinstance(insert, str) else "",
                    }
                )
        else:
...
def markdown_to_quill_delta(md_text: str) -> dict[str, Any]:
    """Markdown → Quill Delta（mistune AST，不经 HTML）。"""
    return markdown_ast_to_quill_delta(md_text)
