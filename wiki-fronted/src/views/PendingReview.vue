<template>
  <div class="applications-page">
    <div class="content-header">
      <h1 class="content-title">{{ translate('applications.pendingReview') }}</h1>
    ...
    <div class="applications-content" :class="{ 'loading-active': loading }">
      ...
      <template v-else>
        <div v-if="list.length === 0" class="empty-state">
          ...
          <p>{{ translate('applications.pendingReviewEmpty') }}</p>
        </div>
        <template v-else>
          <tiny-grid
            :data="list"
          ...
          >
            ...
            <tiny-grid-column :title="translate('applications.actions')" :label="translate('applications.actions')" width="180" align="center" fixed="right">
              <template #default="{ row }">
                <div v-if="row.status === APPLICATION_STATUS.PENDING" class="table-actions" @click.stop>
                  <tiny-button
                    type="success"
                    size="small"
                    :loading="reviewingId === row.id && reviewingApproved === true"
                    @click="handleReview(row.id, true)"
                  >
                    {{ translate('applications.approve') }}
                  </tiny-button>
                  <tiny-button
                    type="danger"
                    size="small"
                    :loading="reviewingId === row.id && reviewingApproved === false"
                    @click="handleReview(row.id, false)"
                    style="margin-left: 8px;"
                  >
                    {{ translate('applications.reject') }}
                  </tiny-button>
                </div>
                <span v-else class="text-placeholder">—</span>
              </template>
    ...
    <tiny-dialog-box
      ...
      :title="reviewingApproved ? translate('applications.approve') : translate('applications.reject')"
      width="480px"
    ...
    >
      <tiny-form
        ...
        label-width="100px"
      >
        <tiny-form-item :label="translate('applications.replyMessage')">
          <tiny-input
            v-model="reviewForm.reply_message"
            type="textarea"
            :rows="4"
            :placeholder="translate('applications.replyMessagePlaceholder')"
          />
        </tiny-form-item>
      </tiny-form>
      <template #footer>
        <tiny-button @click="reviewDialogVisible = false">
...
import { Select as TinySelect, Option as TinyOption, Pager as TinyPager, Button as TinyButton, Modal, DialogBox as TinyDialogBox, Form as TinyForm, FormItem as TinyFormItem, Input as TinyInput, Grid as TinyGrid, GridColumn as TinyGridColumn } from '@opentiny/vue'
...
const reviewingApproved = ref<boolean | null>(null)
...
const handleReview = (applicationId: number, approved: boolean) => {
  currentReviewId.value = applicationId
  reviewingApproved.value = approved
  reviewForm.value.reply_message = ''
...
const submitReview = async () => {
  ...
  try {
    ...
    reviewingId.value = currentReviewId.value
    await applicationApi.reviewApplication(currentReviewId.value, {
      approved: reviewingApproved.value === true,
      reply_message: reviewForm.value.reply_message.trim() || null
    })
    Modal.message({
      message: translate('applications.reviewSuccess'),
      status: 'success'
    })
    ...
    reviewingApproved.value = null
    await fetchList()
  } catch (e: any) {
    Modal.message({
      message: e?.message || translate('applications.reviewError'),
      status: 'error'
    })
  } finally {
