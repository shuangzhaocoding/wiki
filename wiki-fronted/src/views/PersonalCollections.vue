<template>
  <div class="activity-list-page">
    <div class="content-header">
      <h1 class="content-title">{{ translate('personalCenter.collections') }}</h1>
    </div>
    <div class="activity-content" :class="{ 'loading-active': loading }">
      ...
      <template v-else>
        <div v-if="list.length === 0" class="empty-state">
          <p>{{ translate('personalCenter.collectionsEmpty') }}</p>
        </div>
        <template v-else>
          <div class="activity-list">
            ...
            >
              <div class="item-icon collections">
              ...
              <div class="item-actions">
                ...
                >
                  {{ translate('personalCenter.uncollect') }}
                </button>
...
const handleUncollect = async (item: UserActivityItem) => {
  const { articleId } = parseActivityItem(item)
  if (!articleId) return
  try {
    removingId.value = articleId
    await articleApi.uncollectArticle(articleId)
    Modal.message({ message: translate('personalCenter.uncollectSuccess'), status: 'success' })
    await fetchList()
  } catch (e: any) {
...
const fetchList = async () => {
  try {
    loading.value = true
    const { items, total: t } = await userApi.getMyCollections(currentPage.value, pageSize.value)
    list.value = items
    total.value = t
  } catch (e: any) {
    Modal.message({ message: e?.message || translate('personalCenter.fetchError'), status: 'error' })
